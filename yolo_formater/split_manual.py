...
def manualsplit(
    ...
    for img, lbl in tqdm(img_lbl_file_pth_train):
        ...
        shutil.copy(
            lbl, os.path.join(out_folder, "Train", "labels", os.path.basename(lbl))
        )
        shutil.copy(
            img, os.path.join(out_folder, "Train", "images", os.path.basename(img))
        )

    print("Creating val dataset")
    for img, lbl in tqdm(img_lbl_file_pth_val):
        shutil.copy(
            lbl, os.path.join(out_folder, "Val", "labels", os.path.basename(lbl))
        )
        shutil.copy(
            img, os.path.join(out_folder, "Val", "images", os.path.basename(img))
        )

    print("Creating test dataset")
    for img, lbl in tqdm(img_lbl_file_pth_test):
        shutil.copy(
            lbl, os.path.join(out_folder, "Test", "labels", os.path.basename(lbl))
        )
        shutil.copy(
            img, os.path.join(out_folder, "Test", "images", os.path.basename(img))
        )
